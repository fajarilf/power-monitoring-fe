...
export function KpiGrid() {
  ...
  return (
    <div className="grid grid-cols-1 gap-3.5 sm:grid-cols-2 lg:grid-cols-4">
      <StatCard label="Energy (WP+)" value={(reading?.kwh ?? 0) / 10} unit="MWh" accent="var(--phase-t)">
        {statusLine}
      </StatCard>
      ...
      <StatCard label="Power Factor" value={(reading?.pf ?? 0) / 1000} unit="" digits={3} accent="var(--good)">
        {reading ? (
